...
3. ~~**Re-wire the Signal Archive.**~~ ✅ **Shipped 2026-06-12.** "Track Record" collapsible section in This Week, backed by a new `/api/signals/history` endpoint over `signal_history.json` (the real append-only archive with scored outcomes) — past calls grouped by week with ✅/❌/⏳ badges, long-term outcomes shown vs SPY, and an explicit "we publish before we know the results" framing while the first cohort matures. Orphaned `SignalArchive.jsx` (which read the non-archival current store) deleted; `/api/signals/archive` left untouched (the daily capture job depends on it).
...
| 2026-06-12 | **P0-3 shipped:** Signal Archive re-wired as the "Track Record" section in This Week — past calls with scored outcomes from `signal_history.json` via new `/api/signals/history`; all P0 items now complete |
| 2026-06-12 | **Per-timeframe cadence + honest accuracy windows + differentiated cards:** short-term regenerates >6h, long-term >24h (judged per-signal; a rate-limited pass retries hourly instead of waiting a full cycle); accuracy now scores short-term at 30d absolute and long-term at 90d **vs SPY** (legacy untagged entries keep 30d-absolute so the existing cohort still matures ~2026-07-11); short-term cards show momentum/earnings/range badges, long-term cards show P/E + dividend badges |
